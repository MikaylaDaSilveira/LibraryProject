...
function addBookInfoItemToDisplay(item) {
    let dv = document.getElementById("bookinfoList");
    dv.innerText = dv.innerText + ' , ' + item["bookTitle"];
    //let bookinfoItemNode = document.createElement("li");
    //let descriptionTextNode = document.createTextNode(item["bookTitle"]);
...
function readBookInfo() {
    fetch('https://localhost:7202/bookinfo')
        // get the JSON content from the response
        .then((response) => {
            if (!response.ok) {
...
function updateBookInfoItem(item) {
    ...
    fetch('https://localhost:7202/bookinfo/' + item.id, requestOptions)
        .then((response) => {
            if (!response.ok) {
...
function deleteBookInfoItem(bookinfoItemId) {
    fetch('https://localhost:7202/bookinfo/' + bookinfoItemId, {method: 'DELETE'})
            .then((response) => {
        if (!response.ok) {
